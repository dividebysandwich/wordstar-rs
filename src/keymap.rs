...
fn resolve_idle(state: &mut ChordState, key: KeyEvent) -> Resolution {
    ...
    match c {
        ...
        'q' => {
            *state = ChordState::Q;
            Resolution::Pending(
                "^Q  Quick:  S/D line ends  R/C file ends  I)page  G)heading  P)revious  0-9 marker  B/K block ends  F)ind  A)replace  L)spelling  N)spell word  J)thesaurus  Y)del line end  M)calc",
            )
        }
        'o' => {
            *state = ChordState::O;
...
fn resolve_q(key: KeyEvent) -> Resolution {
    ...
    match letter(&key) {
        ...
        Some('l') => Resolution::Command(SpellCheck),
        Some('n') => Resolution::Command(SpellWord),
        Some('j') => Resolution::Command(Thesaurus),
